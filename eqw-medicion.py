...
while (loop_sigma==True):
    ...
    p, C_p = np.polyfit(lambda_new_s, si_back_s, n, cov=True)  # C_z is estimated covariance matrix
    ...
    plt.scatter(t, ss,marker='x',c='orange')
    plt.scatter(t[indicesig],ss[indicesig],c='green',marker='x')
...
vec_to_sort=np.c_[lambnew2,el_new2,ep_new2,loggf_new2,damp_new2,disp2,infoeqwNew2] #sort for element
indx_srt=np.argsort(vec_to_sort[:,1])
final_vec=vec_to_sort[indx_srt,:]

########################   Save measurements with MOOG format ##############################################################
formato="%4.3f %2.1f %2.3f %2.3f  %2.3f %1.1f %3.1f" 
np.savetxt(name_out+'.'+el_name+'.MOOG.dat',final_vec,fmt=formato)  #output file
#####################################################################################
##################################### END ################################################
#####################################################################################
#####################################################################################
